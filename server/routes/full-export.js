...
const TABLES = [
  ...
  // Vendor identity as it stands today (aliases) and how it got there (the
  // merge log). Without the log an export shows one vendor where the workspace
  // once had three, with nothing saying so.
  { file: 'vendor_aliases.csv', sql: 'SELECT id,canonical,alias,created_by,created_at FROM vendor_aliases WHERE label_id=$1 ORDER BY canonical,alias', cols: ['id', 'canonical', 'alias', 'created_by', 'created_at'] },
  { file: 'vendor_emails.csv', sql: 'SELECT id,vendor,email,label_text,created_by,created_at FROM vendor_emails WHERE label_id=$1 ORDER BY vendor,email', cols: ['id', 'vendor', 'email', 'label_text', 'created_by', 'created_at'] },
  { file: 'vendor_merges.csv', sql: 'SELECT id,kind,from_name,into_name,created_alias,merged_by,merged_at,undone_at,undone_by FROM vendor_merge_log WHERE label_id=$1 ORDER BY merged_at DESC', cols: ['id', 'kind', 'from_name', 'into_name', 'created_alias', 'merged_by', 'merged_at', 'undone_at', 'undone_by'] },
  ...
  { file: 'recording_budgets.csv', sql: 'SELECT id,artist_id,release_id,artist_name,project_title,type,currency,advance_amount,fund_amount,proposed_tracks,contingency_pct,status,notes,created_by,approved_by,approved_at,locked_by,locked_at,updated_at FROM recording_budgets WHERE label_id=$1 ORDER BY id', cols: ['id', 'artist_id', 'release_id', 'artist_name', 'project_title', 'type', 'currency', 'advance_amount', 'fund_amount', 'proposed_tracks', 'contingency_pct', 'status', 'notes', 'created_by', 'approved_by', 'approved_at', 'locked_by', 'locked_at', 'updated_at'] },
  { file: 'recording_budget_items.csv', sql: 'SELECT id,budget_id,section,description,category,qty,unit_price,amount,notes,sort_order FROM recording_budget_items WHERE label_id=$1 ORDER BY budget_id,section,sort_order,id', cols: ['id', 'budget_id', 'section', 'description', 'category', 'qty', 'unit_price', 'amount', 'notes', 'sort_order'] },
  { file: 'funding_sources.csv', sql: "SELECT id,name,kind,reimbursable,active,created_at FROM funding_sources WHERE label_id=$1 ORDER BY id", cols: ['id', 'name', 'kind', 'reimbursable', 'active', 'created_at'] },
