...
export default function SpendByArtist({ data, onDrill }) {
  ...
  return (
    <div>
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="text-[11px] uppercase tracking-wide text-gray-400">
              ...
              {cats.map((c) => <th key={c} className="px-3 py-2 text-right font-semibold whitespace-nowrap">{c}</th>)}
            </tr>
          </thead>
          <tbody>
            {shown.map((r) => (
              <tr key={r.key} className="border-t border-divider">
                <td className={`px-3 py-1.5 truncate max-w-[220px] ${STICKY_TD}`}>{r.name}</td>
                ...
                {cats.map((c) => (
                  <td key={c} className="px-3 py-1.5 text-right tabular-nums">
                    {r.by_category[c] ? <button className="hover:underline" onClick={() => drillArtist(r, c)}>{money(r.by_category[c])}</button> : <span className="text-gray-300">—</span>}
                  </td>
                ))}
              </tr>
            ))}
            ...
            {unattributed && (
              <tr className="border-t border-divider text-gray-500 italic">
                ...
                {cats.map((c) => (
                  <td key={c} className="px-3 py-1.5 text-right tabular-nums">
                    {unattributed.by_category[c] ? <button className="hover:underline" onClick={() => drillArtist(unattributed, c)}>{money(unattributed.by_category[c])}</button> : '—'}
                  </td>
                ))}
              </tr>
            )}
      ...
      <div className="card p-4 text-sm">
        ...
        <p className="text-[11px] text-gray-400 mt-2">Placeholder artist values (N/A, TBD, various…) count as unattributed, never as artists. The rollup ties to the P&L expense total by construction.</p>
      </div>
    </div>
  )
}
