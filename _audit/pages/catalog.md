...
- Year sort: numeric `b - a` (OLD :249) vs `String localeCompare` desc (NEW :61) — equivalent for 4-digit years; 'Undated' sorts after digits in NEW, OLD's 'Unknown' order under numeric-minus is NaN-dependent (RESOLVED 2026-09-02 (Phase 10) — the comparator is numeric descending, not `localeCompare`: `Catalog.jsx:165-171` is `if (x === 'Undated') return 1; if (y === 'Undated') return -1; return Number(y) - Number(x)`. 'Undated' sorts LAST, deterministically. The two `localeCompare` calls at `:126`/`:135` are artist/genre dropdowns, not years).
