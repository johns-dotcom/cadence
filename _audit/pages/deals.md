...
| DEF-DEALS-10 | P2 | `Deals.jsx:112-114,126-127` | Drag plumbing lost `dragCounters` enter/leave ref, `isDifferentStage` gating, `effectAllowed/dropEffect` — child-element hover flicker STILL UNVERIFIED — needs a browser (see the note at §4); the missing `dragCounters`/`effectAllowed` plumbing is source-confirmed, only the resulting flicker is not | `DealPipeline.jsx:136,196-231` |
