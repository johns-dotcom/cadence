...
- Refetch on acting-user switch — OLD `useEffect(fetchData, [user?.id])` (:461); NEW `useTaskData` reloads only on surface change (useTaskData.js:57-66). `UNVERIFIED — needs runtime check` whether cadence impersonation remounts the tree.
...
18. **[P3] No refetch on acting-user switch** — OLD refetches on `user?.id` (:461); NEW loads on surface change only (useTaskData.js:57-66). `UNVERIFIED — needs runtime check` (impersonation may remount the tree). Fix: add user id to `useTaskData` deps. (LOW)
